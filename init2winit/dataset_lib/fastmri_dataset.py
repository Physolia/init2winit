...
def _process_example(kspace, kspace_shape, target, target_shape, seed):
  ...
  Args:
    kspace: raw mri data.
    kspace_shape: shape of kspace. We pass this in
  ...
  # normalize_instance
  mean, std = tf.math.reduce_mean(abs_image), tf.math.reduce_std(abs_image)
...
def _create_generator(filename):
  tf.print('FastMRI h5 paths:', filename)
  signature = (
...
def load_split(per_host_batch_size, split, hps, shuffle_rng=None):
  ...
  if split in ['train', 'eval_train']:
    ...
    data_dir = os.path.join(data_dir, hps.valid_dir)
  ...
  def process_example(example_index, example):
    process_rng = tf.cast(shuffle_rng, tf.int64)
    if split == 'train':
